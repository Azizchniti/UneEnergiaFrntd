...
//const BASE_URL = 'http://localhost:3000/api/education';
...
export const EducationService = {
  ...
  async createCertification(data: Partial<Certification>): Promise<Certification> {
    const res = await axios.post(`${API_URL}/certifications`, data);
    return res.data;
  },

  async updateCertification(id: string, data: Partial<Certification>): Promise<Certification> {
    const res = await axios.put(`${API_URL}/certifications/${id}`, data);
    return res.data;
  },
  ...
  async createPath(data: Partial<LearningPath>): Promise<LearningPath> {
    const res = await axios.post(`${API_URL}/paths`, data);
    return res.data;
  },

  async updatePath(id: string, data: Partial<LearningPath>): Promise<LearningPath> {
    const res = await axios.put(`${API_URL}/paths/${id}`, data);
    return res.data;
  },

  async deletePath(id: string): Promise<void> {
    await axios.delete(`${API_URL}/paths/${id}`);
  },
    // === COURSES ===
  ...
  const res = await axios.post(`${API_URL}/classes/by-ids`, { ids });
  return res.data;
}
,
